    ...
    if ($selected_contact_id > 0) {
        ...
        if ($selected_contact) {
            // ✅ جلب الرسائل باستخدام اسم العمود الصحيح (message)
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
        ...
        if ($receiver_id <= 0 || $message_text === '') {
            ...
            // ✅ اسم العمود الصحيح في قاعدة البيانات هو (message)
            ...
            if ($insert_stmt->execute()) {
                ...
                // ✅ إعادة تحميل الصفحة لإظهار الرسالة الجديدة
            // header("Location: /student_portal/teacher/chat.php?contact_id=$receiver_id");
                exit();
            } else {
                $send_error = 'Error sending message: ' . $conn->error;
            }
        }
    }
    ?>
            ...
            <aside class="sidebar">
                ...
                <nav>
                    <ul class="sidebar-menu">
                        ...
                        <li><a href="/student_portal/teacher/chat.php" class="active"><i class="fas fa-comments"></i> Chat</a></li>
                                ...
                                <?php if (!empty($contacts)): ?>
                                    <?php foreach ($contacts as $contact): ?>
                                        <div class="contact-item <?php echo $contact['id'] == $selected_contact_id ? 'active' : ''; ?>"
                                            onclick="location.href='/student_portal/teacher/chat.php?contact_id=<?php echo $contact['id']; ?>'">
                                    ...
                                    <div class="chat-header">
                                        <div style="display: flex; align-items: center; gap: 0.75rem;">
                                            ...
                                            <div>
                                                ...
                                                <small style="color: var(--text-light);">Parent</small>
                                            </div>
                                        </div>
                                    </div>
        ...
        messageForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success && data.message_data) {
                    ...
                    // إضافة الرسالة مباشرةً إلى الـ DOM
                    const container = document.getElementById('messagesArea');
                    const div = document.createElement('div');
                    div.className = 'message sent';
                    div.setAttribute('data-id', data.message_data.id);
                    div.innerHTML = `
                        <div class="message-content">
                            ${data.message_data.message}
                            <div class="message-time">${data.message_data.created_at}</div>
                        </div>
                    `;
